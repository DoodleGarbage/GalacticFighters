...
func get_placements() -> Array[Vector2]:
	...
	for card in assigned_cards:
		...
		new_placement.x += scale.x * distance_on_row * (card.manager_position % max_p_row)
		@warning_ignore("integer_division")
		new_placement.y += 500 * scale.y * card_spacing.y * floor(card.manager_position / max_p_row) + step * scale.y * (card.manager_position % max_p_row)
